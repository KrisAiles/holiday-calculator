...
if (currentMonth >= 0 && currentMonth <= 7) {
    currentPartOne = currentYear - 1;
    currentPartTwo = currentYear;
    nextPartOne = currentYear;
    nextPartTwo = currentYear + 1;
} else {
    currentPartOne = currentYear;
    currentPartTwo = currentYear + 1;
    nextPartOne = currentYear + 1;
    nextPartTwo = currentYear + 2;
}

const currentText = document.getElementById('current');
currentText.textContent = `${currentPartOne}/${currentPartTwo}`;
const nextText = document.getElementById('next');
nextText.textContent = `${nextPartOne}/${nextPartTwo}`;

let bankHolsArray = [];

async function getBankHols(year) {
    bankHolsArray = [];
    let yearPartOne;
    let yearPartTwo;
    if (year === 'current') {
...
proRataHoursForm.addEventListener("submit", (e) => {
    e.preventDefault();
    // Select holiday year
    const bankHolsYear = document.getElementById("year").value;//-------------------------------------------------------------------------------------<< input current holiday year
    // Add employee name
    ...
    const ptHolEntArray = [];    
    async function calculateHolidayEntitlement(startMonthNum, startDay, startMonth) {
    ...
    async function callCalculateHolidayEntitlement() {
    // call the function based on the start month input
    await calculateHolidayEntitlement(monthNumber, inputStartDay, inputStartMonth);
    // call the function with the full 12 months
    await calculateHolidayEntitlement(12, 1, "September");
